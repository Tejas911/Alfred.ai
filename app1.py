...
def main():
    st.set_page_config(
        ...
        page_icon="assets\app_logo.jpeg",
        layout="wide",
        initial_sidebar_state="expanded",
        menu_items={"About": "# This is a header. This is an *extremely* cool app!"},
    )

    logo_url = "assets\app_logo.jpeg"
    ...
    if user_question := st.chat_input("Ask a Question from the PDF Files"):
        ...
        st.chat_message(
            "human", avatar="https://i.postimg.cc/261JMMfm/user-3.png"
        ).write(user_question)
        answer, context = user_input(user_question, selected_model_name)
        st.chat_message(
            "ai", avatar="https://i.postimg.cc/fLSW0H9V/chat-16273634.png"
        ).write(answer)
        st.session_state.history.append(
            {
                "type": "human",
                "content": user_question,
                "avatar": "https://i.postimg.cc/261JMMfm/user-3.png",
            }
        )
        st.session_state.history.append(
            {
                "type": "ai",
                "content": answer,
                "avatar": "https://i.postimg.cc/fLSW0H9V/chat-16273634.png",
            }
        )
    ...
    if st.session_state.last_question:
        if st.button("Regenerate"):
            st.chat_message(
                "human", avatar="https://i.postimg.cc/261JMMfm/user-3.png"
            ).write(st.session_state.last_question)
